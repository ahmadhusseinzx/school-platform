// src/components/admin/ImportStudents.jsx
...
export default function ImportStudents({ onImportComplete }) {
  ...
  const downloadTemplate = () => {
    const template = [
      { 
        'الاسم الكامل': 'أحمد محمد الطالب',
        'رقم الهوية': '123456789',
        'مكان الولادة': 'القدس',
        'الجنسية': 'فلسطين',
        'مكان السكن': 'العيزرية',
        'رقم الهاتف': '0591234567',
        'هاتف ولي الأمر': '0597654321',
        'تاريخ الميلاد': '2018-09-25'
      },
      { 
        'الاسم الكامل': 'سارة خالد العلي',
        'رقم الهوية': '987654321',
        'مكان الولادة': 'رام الله',
        'الجنسية': 'فلسطين',
        'مكان السكن': 'رام الله',
        'رقم الهاتف': '0592345678',
        'هاتف ولي الأمر': '0598765432',
        'تاريخ الميلاد': '2017-05-12'
      },
      { 
        'الاسم الكامل': 'محمد عبد الله السالم',
        'رقم الهوية': '456789123',
        'مكان الولادة': 'الخليل',
        'الجنسية': 'فلسطين',
        'مكان السكن': 'الخليل',
        'رقم الهاتف': '0593456789',
        'هاتف ولي الأمر': '0599876543',
        'تاريخ الميلاد': '2019-01-15'
      },
    ];
    ...
    XLSX.writeFile(wb, 'استيراد_طلاب_مثال.xlsx');
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setMessage({ type: 'info', text: `✅ تم قراءة ${jsonData.length} طالب من الملف` });
      } catch (err) {
  ...
  const handleImport = async () => {
    ...
    for (const row of previewData) {
      ...
      const idNumber = row['رقم الهوية']?.toString().trim() || '';
      const birthPlace = row['مكان الولادة']?.trim() || '';
      const nationality = row['الجنسية']?.trim() || '';
      const address = row['مكان السكن']?.trim() || '';
      const phone = row['رقم الهاتف']?.toString().trim() || '';
      const parentPhone = row['هاتف ولي الأمر']?.toString().trim() || '';
      const dateOfBirth = row['تاريخ الميلاد']?.toString().trim() || '';
    ...
    setMessage({ 
      ...
      text: `✅ تم استيراد ${results.success} طالب${results.failed > 0 ? `، فشل ${results.failed} طالب` : ''}`
    });
  ...
  const exportResults = () => {
    const exportData = results.details.map(item => ({
      ...
      'مكان الولادة': item.birthPlace || '',
      'الجنسية': item.nationality || '',
      'مكان السكن': item.address || '',
    ...
    XLSX.writeFile(wb, 'نتائج_الاستيراد.xlsx');
  };
    ...
    <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">قم بتحميل ملف Excel يحتوي على بيانات الطلاب للاستيراد التلقائي</p>
        </div>
      ...
      {/* ====== اختيار الصف ====== */}
      <div className="mb-4">
        <label className="block text-xs font-bold text-slate-400 mb-2">اختر الصف الدراسي *</label>
          ...
          <label htmlFor="file-upload" className="cursor-pointer">
            ...
            {file && (
              <p className="text-xs text-emerald-400 mt-2">
                ✅ تم تحميل الملف: {previewData.length} طالب
              </p>
            )}
          </label>
        </div>
            ...
            <table className="w-full text-right text-xs">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="p-2 text-slate-400">رقم الهوية</th>
                  <th className="p-2 text-slate-400">مكان الولادة</th>
                  <th className="p-2 text-slate-400">الجنسية</th>
                  <th className="p-2 text-slate-400">مكان السكن</th>
                </tr>
              </thead>
              <tbody>
                {previewData.slice(0, 10).map((row, index) => (
                  <tr key={index} className="border-b border-slate-800">
                    ...
                    <td className="p-2 text-slate-300">{row['رقم الهوية'] || '-'}</td>
                    <td className="p-2 text-slate-300">{row['مكان الولادة'] || '-'}</td>
                    <td className="p-2 text-slate-300">{row['الجنسية'] || '-'}</td>
                    <td className="p-2 text-slate-300">{row['مكان السكن'] || '-'}</td>
                  </tr>
                ))}
                {previewData.length > 10 && (
                  <tr>
                    <td colSpan={6} className="p-2 text-center text-slate-500">
                      ... و {previewData.length - 10} طالب آخر
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        >
          {importing ? (
            <>
              ...
              جاري استيراد {previewData.length} طالب...
            </>
          ) : (
            <>
          ...
          <div className="overflow-x-auto max-h-60">
            <table className="w-full text-right text-xs">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="p-2 text-slate-400">#</th>
                  <th className="p-2 text-slate-400">اسم الطالب</th>
              ...
              <tbody>
                {results.details.map((item, index) => (
                  <tr key={index} className="border-b border-slate-800">
                    ...
                    <td className="p-2 text-slate-300">{item.idNumber || '-'}</td>
                    <td className="p-2">
                      {item.status === 'success' ? (
